...
import { GET_ACTIVITIES } from "../../api/myQueries";
...
export default function SubActivitySelectBox({ label, name, isEmpty, value, handleSubtaskSelect }:SelectBox){
    const {data, loading, error} = useQuery(GET_ACTIVITIES);
    const handleSelectedMaterial = (evt: React.ChangeEvent<HTMLSelectElement>) => {}

    if(data){
        let activitiesList = data.subActivitiesList as ActivityProps[];
